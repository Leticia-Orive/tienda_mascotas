/**
 * Archivo de datos iniciales para próximos productos
 *
 * QUÉ ES:
 * Define el array UPCOMING_PRODUCTS con los datos hardcodeados de próximos lanzamientos.
 * Actúa como seed/datos por defecto que se cargan al iniciar la aplicación.
 *
 * PARA QUÉ SIRVE:
 * - Proporcionar datos iniciales si no hay datos guardados en localStorage
 * - Servir de referencia/ejemplo de la estructura de datos esperada
 * - Permitir resetear a valores por defecto si es necesario
 *
 * FUNCIONALIDAD:
 * - Cada producto tiene: id único (100+), nombre, categoría, descripción,
 *   fecha de lanzamiento, precio estimado e imagen
 * - Se importa en UpcomingProductsService para comparar/validar al cargar
 * - Se renderiza en la Home cuando no hay datos persistidos
 */
import { UpcomingProduct } from '../models/product.model';

export const UPCOMING_PRODUCTS: UpcomingProduct[] = [
  {
    id: 101,
    nombre: 'Arenero Inteligente AutoClean X2',
    categoria: 'accesorios',
    descripcion: 'Sistema autolimpiante con filtro antiolor y control desde app para mantener siempre limpio el espacio de tu gato.',
    lanzamiento: 'Abril 2026',
    precioEstimado: 219.90,
    imagen: '/img/arenero.webp',
  },
  {
    id: 102,
    nombre: 'Snack Dental Funcional ProBite',
    categoria: 'alimento',
    descripcion: 'Premio diario con probioticos y textura especial para reducir placa y cuidar la salud bucal en perros adultos.',
    lanzamiento: 'Mayo 2026',
    precioEstimado: 14.95,
    imagen: '/img/snacks_perro.webp',
  },
  {
    id: 103,
    nombre: 'Fuente de Agua SmartFlow Mini',
    categoria: 'cuidado',
    descripcion: 'Fuente silenciosa con sensor de movimiento y triple filtrado para incentivar la hidratacion de gatos y perros pequenos.',
    lanzamiento: 'Junio 2026',
    precioEstimado: 64.50,
    imagen: '/img/fuente_de_agua.webp',
  },
  {
    id: 104,
    nombre: 'Collar GPS Mascota Segura',
    categoria: 'accesorios',
    descripcion: 'Rastreador GPS en tiempo real con collar ergonómico resistente al agua para mantener localizado a tu mascota siempre.',
    lanzamiento: 'Julio 2026',
    precioEstimado: 89.99,
    imagen: '/img/collar_gps.webp',
  },
  {
    id: 105,
    nombre: 'Juguete Interactivo Kong SmartPlay',
    categoria: 'juguetes',
    descripcion: 'Juguete rellenable con dispensador automático de premios para horas de entretenimiento y estimulación mental.',
    lanzamiento: 'Agosto 2026',
    precioEstimado: 34.50,
    imagen: '/img/kong_smartplay.webp',
  },
  {
    id: 106,
    nombre: 'Cama Memory Foam Deluxe',
    categoria: 'cuidado',
    descripcion: 'Cama ortopédica con espuma viscoelástica y funda extraíble lavable para máxima comodidad y apoyo articular.',
    lanzamiento: 'Septiembre 2026',
    precioEstimado: 129.99,
    imagen: '/img/cama_memory.webp',
  },
];
